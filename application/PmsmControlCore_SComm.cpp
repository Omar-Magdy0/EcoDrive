...
void PmsmControlCore::SComm_onEnter(MCType prev_mct)
{
...
void PmsmControlCore::SComm_xmcLoop()
{
    ...
    case SComm::IDStage::LD_ID:
        ...
        case SComm::IDSubStage::Active_Sampling:
            if (scomm.samples_counter == 0)
            {
                ...
                float Xd = sqrt(Zd*Zd - scomm.R*scomm.R);
    ...
    case SComm::IDStage::LQ_ID:
        ...
        case SComm::IDSubStage::Active_Sampling:
            if (scomm.samples_counter == 0)
            {
                q31_t Iqd = (scomm.accumulate0 >> (scomm.oversample_bits) );// Since accumulator accumulates I/2 we compensate in shift 
                q31_t Iqq = (scomm.accumulate1 >> (scomm.oversample_bits) );// Since accumulator accumulates I/2 we compensate in shift
                ...
                float Xq = sqrt(Zq*Zq - scomm.R*scomm.R);
                scomm.Lq = (Xq)/(angle_q31_to_float(scomm.hfi_Angv_RPT_q31)*pwm_freq_hz);
                scomm.idsub = SComm::IDSubStage::ESettle_Start;
                scomm.idstage = SComm::IDStage::RS_ID;
            }
            break;
