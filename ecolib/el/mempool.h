...
extern "C"
{
    ...
    namespace el
    {
        ...
        static inline void el_smem_init(smem_t *cp, uint8_t *mem, uint32_t memsize)
        {
            cp->mem = mem;
            cp->offset = 0;
            cp->memsize = memsize;
        ...
        static inline void *el_smem_alloc(smem_t *cp, uint32_t size)
        {
...
#define ALIGN4(x) (((x) + 3) & ~3)
        ...
        static inline void *el_smem_alloc_aligned(smem_t *cp, uint32_t size)
        {
